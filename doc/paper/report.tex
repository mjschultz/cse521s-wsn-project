...
The TelosB includes many different interfaces we could use to attach sensors too.  These include analog to digital, UART, I2C, and digital connections, this selection gives us a very flexible platform~\cite{moteiv:telosb}.  Its this flexibility that will make this system a viable option for parking structures.
...
because of it's low price and large feature set.
The EZ1 supports a wide range of standard supply voltage inputs.  This allows us a great deal of flexibility in choosing how to power the EZ1.  It can detect objects from 6 inches to 254 inches.  This is enough range to cover the complete parking space.  The EZ1 supports many output formats including pulse width, analog voltage, and serial digital.  This gives us multiple ways to connect the same sensor depending on the environment~\cite{maxbotix:maxsonar-datasheet}.
...
The base-station software has multiple functions.  It was written in C so as to be compatible with most current versions of unix/linux.  It is responsible for configuring the PSMs, monitoring their status, collecting data from them, and sending the collected data to the aggregation software over the internet.
...
PSMs.  Its through this configuration process that the PSMs are assigned their parking space IDs.  The configuration packet structure was designed to be easily extended to support future needs.  For example, the base-station could adjust sensor read rates or data transmission rates based on the time of day to increase PSM battery life.

The base-station's primary purpose is to collect sensor readings from the PSMs
and then determines what data is required by the aggregation software.  That data is then JSON encoded and transfered via an HTTP put to the Google App Engine were it can be processed by the aggregation software.  
...
with the sensing range of our PSMs.
The EZ1 claims to support a distance up to 254-inches, however we
...
motorcycles.  More testing and analysis would need to be performed to
determine if the ten foot range of the EZ1 would really be an issue.  It was determined while testing that the issue
was with the supply voltage to the EZ1.
The EZ1 range finder will support a supply voltage of 2.5V - 5.5V.
We chose to power the EZ1 using the TelosB analog supply which can
only supply about 3.1V.
When we used an external supply to test just the EZ1 at around 5V
it had a much greater range compared to 3.1V.
If after more testing it is determined to be an issue, there are solutions
such as using an external power supply for the EZ1 or possibly
...
being a two wire digital serial connection.  This allows for up to twelve EZ1s to be used with a single serial connection.  This would allow us to either use two sensors per space to improve accuracy
